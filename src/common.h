...
struct ExtraFeatures {
    ...
    bool enableSeeSpline = false; 
    bool enableSeeTimes = false;
    uint32_t numBlurSamples = 10;
    std::vector<glm::vec3> splinePoints = { { 0, 0, 0 }, { 0, 0, -1 }, { 0, 0, -2 }, { 0, 0, -3 } };

    // Parameters for Environment Mapping
    float environmentScale;
    // Parameters for Bloom effect
    float bloom_filter_size;
    float bloom_filter_threshold;
    uint32_t bloom_filter_scale;
    bool brightnessOnly = false;
    bool visualizeGlossyDiskDebug = false;
...
struct Features {
    ...
    bool enableMultiSamplingDebug = false;
