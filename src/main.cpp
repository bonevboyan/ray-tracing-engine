        ...
        while (!window.shouldClose()) {
            ...
            if (ImGui::CollapsingHeader("Extra Features")) {
                ...
                if (config.features.extra.enableBloomEffect) {
                    ...
                    uint32_t minSamples = 3, maxSamples = 30;
                    ImGui::SliderScalar("Size", ImGuiDataType_U32, &config.features.extra.bloom_filter_size, &minSamples, &maxSamples);
                    ImGui::DragFloat("Threshold", &config.features.extra.bloom_filter_threshold, 0.0f, 1.0f);
                    uint32_t minScale = 1, maxScale = 8;
                    ImGui::SliderScalar("Scale", ImGuiDataType_U32, &config.features.extra.bloom_filter_scale, &minScale, &maxScale);
                    ImGui::Checkbox("Debug Brightness Only", &config.features.extra.brightnessOnly);
                    ImGui::Unindent();
                }
                ...
                if (config.features.extra.enableMotionBlur) {
                    ImGui::Indent();
                    uint32_t minSamples = 10u, maxSamples = 1000u;
                    ImGui::SliderScalar("Samples", ImGuiDataType_U32, &config.features.extra.numBlurSamples, &minSamples, &maxSamples);
                    ImGui::Checkbox("Render spline", &config.features.extra.enableSeeSpline);
                    ImGui::Checkbox("Render at discrete time slices", &config.features.extra.enableSeeTimes);

                    ImGui::Text("Edit Spline Points:");

                    for (size_t i = 0; i < config.features.extra.splinePoints.size(); ++i) {
                        ImGui::PushID(static_cast<int>(i));
                        ImGui::Text("Point %d", static_cast<int>(i + 1));
                        ImGui::InputFloat3("Position", &config.features.extra.splinePoints[i].x, "%.3f", ImGuiInputTextFlags_None);
                        ImGui::PopID();
                    }
                ...
                if (config.features.extra.enableGlossyReflection) {
                    ...
                    ImGui::Checkbox("Debug Disk", &config.features.extra.visualizeGlossyDiskDebug);
                    ImGui::Unindent();
                }
                ImGui::Checkbox("Environment maps", &config.features.extra.enableEnvironmentMap);
                if (config.features.extra.enableEnvironmentMap) {
                    ImGui::Indent();
                    ImGui::SliderFloat("Distance (50 = infinity)", &config.features.extra.environmentScale, 1.0f, 50.0f, "%.1f");
                    ImGui::Unindent();
                    ImGui::Indent();
                    ImGui::Checkbox("Environment Cube", &config.features.extra.environmentCube);
                    ImGui::Unindent();
            ...
            if (viewMode == ViewMode::Rasterization) {
                ...
                ImGui::Checkbox("Enable Debug MultiSampling Draw", &config.features.enableMultiSamplingDebug);
            ...
            case ViewMode::Rasterization: {
...

                {
                    ...
                    RenderState state = { .scene = scene, .features = config.features, .bvh = bvh, .sampler = { debugRaySeed } };
                    if (state.features.enableMultiSamplingDebug) {
                        visualizeMultiSampling(state);
                    }
                }
                glPopAttrib();
                ...
                if (config.features.extra.enableSeeSpline) {
                    drawSpline(config.features.extra.splinePoints, {0.0f, 0.0f, 0.0f});
                }
                if (config.features.extra.enableSeeTimes) {
                    drawSceneAtTime(scene, config.features, camera, screen, 0.0f);
                    drawSceneAtTime(scene, config.features, camera, screen, 0.25f);
                    drawSceneAtTime(scene, config.features, camera, screen, 0.5f);
                    drawSceneAtTime(scene, config.features, camera, screen, 1.0f);
                }
            } break;
