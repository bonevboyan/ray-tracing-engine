...
glm::vec3 computeBarycentricCoord(const glm::vec3& v0, const glm::vec3& v1, const glm::vec3& v2, const glm::vec3& p)
{
    ...
    float area = glm::length(glm::cross(v0v1, v0v2)) * 0.5f;

    float areaAlpha = glm::length(glm::cross(v1 - p, v2 - p)) * 0.5f;
    float areaBeta = glm::length(glm::cross(v2 - p, v0 - p)) * 0.5f;
    float areaGamma = glm::length(glm::cross(v0 - p, v1 - p)) * 0.5f;
