...
std::vector<Ray> generatePixelRaysUniform(RenderState& state, const Trackball& camera, glm::ivec2 pixel, glm::ivec2 screenResolution)
{
    ...
    for (int i = 0; i < numSamples; ++i) {
        const glm::vec2 position = (glm::vec2(pixel) + state.sampler.next_2d()) / glm::vec2(screenResolution) * 2.f - 1.f;
...
std::vector<Ray> generatePixelRaysStratified(RenderState& state, const Trackball& camera, glm::ivec2 pixel, glm::ivec2 screenResolution)
{
    ...
    for (int i = 0; i < numSamples; ++i) {
        for (int j = 0; j < numSamples; ++j) {
            const glm::vec2 t = glm::vec2(i / numSamples, j / numSamples);
            const glm::vec2 position = (glm::vec2(pixel) + state.sampler.next_2d() / (float)numSamples + t) / glm::vec2(screenResolution) * 2.f - 1.f;
            rays.push_back(camera.generateRay(position));
        }
    }
    return rays;
...
void visualizeMultiSampling(RenderState& state) {
    drawSquare({ 0, 0, -1 }, { 1, 0, -1 }, { 1, 1, -1 }, {0, 1, -1});
    ...
    if (state.features.numPixelSamples > 1) {
        if (state.features.enableJitteredSampling) {
            ...
            for (int i = 1; i < numSamples; ++i) {
                drawLine({ 1.0f / numSamples * i, 0, -1.01f }, { 1.0f / numSamples * i, 1, -1.01f });
                drawLine({ 0, 1.0f / numSamples * i, -1.01f }, { 1, 1.0f / numSamples * i, -1.01f });
            }
            for (int i = 0; i < numSamples; ++i) {
                for (int j = 0; j < numSamples; ++j) {
                    const glm::vec2 t = glm::vec2((float)i / numSamples, (float)j / numSamples);
                    const glm::vec2 position = state.sampler.next_2d() / (float)numSamples + t;
                    drawSphere({ position.x, position.y, -1.01 }, 0.01, { 0, 1, 0 });
                }
            }
        } else {
            auto numSamples = state.features.numPixelSamples;
            std::vector<Ray> rays;
            for (int i = 0; i < numSamples; ++i) {
                const glm::vec2 position = (state.sampler.next_2d());
                drawSphere({ position.x, position.y, -1.01 }, 0.01, { 0, 1, 0 });
            }
        }
    } else {
